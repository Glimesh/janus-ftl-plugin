...
#define JANUS_STREAMING_DESCRIPTION		"This is an FTL streaming plugin."
	...
	while(!g_atomic_int_get(&stopping) && !g_atomic_int_get(&mountpoint->destroyed)) {
		...
		for(i=0; i<num; i++) {
			if(fds[i].revents & (POLLERR | POLLHUP)) {
				...
				bytes = recvfrom(video_fd[0], buffer, 1500, 0, (struct sockaddr *)&remote, &addrlen);
				...
				else if(rtp->type == 97) { // Audio is payload type 97 for FTL
